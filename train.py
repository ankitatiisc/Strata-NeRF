...
def main(unused_argv):
  ...
  for step, batch in zip(range(init_step, num_steps + 1), pdataset):
    ...
    temp_stats  = stats['losses'].copy()
    for key in temp_stats.keys():
        temp_stats[key] = np.array(temp_stats[key]).tolist()
    
    plot_losses.append(temp_stats)

    if step % 10000==0:
      loss_json_file = os.path.join(config.checkpoint_dir, 'losses_latent.json')
      if os.path.exists(loss_json_file):
        #Load from the existing loss
        with open(loss_json_file) as feedsjson:
            feeds = json.load(feedsjson)
      else:
        feeds = [] #Create an empty list
      feeds.append(plot_losses)
      with open(loss_json_file, mode='w') as f:
