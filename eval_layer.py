...
def main(unused_argv):
  ...
  out_dir = path.join(config.checkpoint_dir,
                      'path_renders' if config.render_path else 'texture_latent_250k')
  ...
  while True:
    ...
    for idx in range(dataset.size):
      ...
      if not config.render_path:
        ...
        m = 'camera_level'
        print(f'{m:30s} = {batch.camera_level:d}')
        for m, v in metric.items():
          print(f'{m:30s} = {v:.4f}')

        metrics.append(metric)
        metrics_cc.append(metric_cc)
        camera_levels.append(batch.camera_level) #Assumption : evaluation is done one by one
        id_camera_levels.add(batch.camera_level)
    ...
    print('Present Camera Levels', id_camera_levels)

    print('\n\n RGB metrics')
    print('='*50)
    for name in metrics[0]:
      scores = [m[name] for m in metrics]
      print('Total \t Metric {}: \t {:.6f}'.format(name, np.mean(scores)))

      #Level Wise metrics
      for level in id_camera_levels:
        scores = [m[name] for i,m in enumerate(metrics) if camera_levels[i]==level]
        print('Level {} \t Metric {}: \t {:.6f}'.format(level, name, np.mean(scores)))
