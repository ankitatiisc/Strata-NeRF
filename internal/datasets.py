...
class Dataset(threading.Thread, metaclass=abc.ABCMeta):
  ...
  def __init__(self,
    ...
    self.camera_level: np.ndarray = None
  ...
  def _make_ray_batch(self,
    ...
    ray_kwargs = {
        ...
        'near': broadcast_scalar(self.near[int(self.camera_level[cam_idx])]),
        'far': broadcast_scalar(self.far[int(self.camera_level[cam_idx])]),
        'cam_idx': broadcast_scalar(cam_idx),
        'camera_level': broadcast_scalar(self.camera_level[cam_idx]),
    }
    ...
    batch = {}
    batch['rays'] = rays
    batch['camera_level'] = self.camera_level[cam_idx]
  ...
  def _next_train(self) -> utils.Batch:
    ...
    if self._batching == utils.BatchingMethod.ALL_IMAGES:
      ...
      # _class_len = self._n_examples//self.num_levels
      # _chosen_lvl = self.iteration_number % _class_len 
      # cam_idx = np.random.randint((_chosen_lvl * _class_len) , ((_chosen_lvl+1) * _class_len), (1,))
  ...
  def generate_ray_batch(self, cam_idx: int) -> utils.Batch:
    ...
    if self._render_spherical:
      ...
      rays = camera_utils.cast_spherical_rays(
          camtoworld, self.height, self.width, self.near[self.camera_level[cam_idx]], self.far[self.camera_level[cam_idx]], xnp=np)
    else:
...
class Blender(Dataset):
  ...
  def _load_renderings(self, config):
    ...
    for _, frame in enumerate(meta['frames']):
      ...
      camera_level.append(frame['camera_level'])

    self.camera_level = np.stack(camera_level, axis=0)
