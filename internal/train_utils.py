...
def cross_entropy_loss(batch, renderings):
  labels = batch.camera_level
  labels_onehot = jax.nn.one_hot(labels, num_classes=3)
  return (optax.softmax_cross_entropy(logits=renderings[1]['camera_level'], labels=labels_onehot).mean()) * 0.0001
...
def create_train_step(model: models.Model,
  ...
  def train_step(
    ...
    def loss_fn(variables):
      ...
      losses = {}
      #pdb.set_trace()
      #losses['camera_level'] = cross_entropy_loss(batch, renderings)
      ...
      #elbo loss
      losses['elbo_loss'] = 0.
      for elem in ray_history:
        losses['elbo_loss'] += elem['elbo_loss']
