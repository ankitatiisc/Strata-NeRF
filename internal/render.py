...
def volumetric_rendering(rgbs,
  ...
  if cam_levels != None:
    #cam_levels_idx = jnp.array([[[x.argmax()] for x in cam_levels[0]]])
    cam_levels = jax.nn.softmax(cam_levels)
    
    cam_level = ((weights[..., None] * cam_levels)).sum(axis = -2)
    rendering['camera_level'] = cam_level
  rendering['rgb'] = rgb
  if compute_extras:
    rendering['acc'] = acc
