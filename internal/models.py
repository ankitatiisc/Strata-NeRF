...
class Model(nn.Module):
  ...
  num_camera_levels = 3
  num_shape_features: int = 256
  num_texture_features: int = 128
  ...
  def __call__(
    ...
    for i_level in range(self.num_levels):
      ...
      # latent_embeddings = functools.partial(nn.Embed, kernel_init=getattr(jax.nn.initializers, self.weight_init)())
      #shape_latent_embeddings = nn.Embed(num_embeddings = self.num_camera_levels, features = self.num_shape_features)
...
class VQ_VAE(nn.Module):
  "A VQ VAE: setting default values "
  def __init__(self, num_codebook_vectors:int, embedding_dim:int, num_camera_levels:int, deg_view:int, fixed_level_latents:bool, disable_level_latents:bool, lambda_vqvae_loss:float, commitment_cost:float=1.):
    super().__init__()
    self.embedding_dim = embedding_dim
    self.num_codebook_vectors = num_codebook_vectors
    self.commitment_cost = commitment_cost
    self.weight_init: str = 'he_uniform'  # Initializer for the weights of the MLP.
    self.deg_view: int = deg_view  # Degree of encoding for viewdirs or refdirs.
    self.fixed_level_latents : bool = fixed_level_latents
    self.num_camera_levels = num_camera_levels
    self.disable_level_latents : bool = disable_level_latents
    self.lambda_vqvae_loss = lambda_vqvae_loss
  @nn.compact
  def __call__(self,
               x,
               camera_level):

    vector_codebook = nn.Embed(num_embeddings = self.num_codebook_vectors, features = self.embedding_dim)
    
    if not self.disable_level_latents:
      if not self.fixed_level_latents:
        level_latent_codebook = nn.Embed(num_embeddings = self.num_camera_levels, features = self.embedding_dim)
      
    dense_layer = functools.partial(
        nn.Dense, kernel_init=getattr(jax.nn.initializers, self.weight_init)())
    #pdb.set_trace()
    if not self.disable_level_latents:
      if self.fixed_level_latents:
        #Encode camera level
        encoded_camera_level = coord.pos_enc(camera_level, min_deg=0, max_deg=self.deg_view, append_identity=True)
        encoded_camera_level_1 = dense_layer(self.embedding_dim)(encoded_camera_level)
      else:
        #Getting latent from embedding
        encoded_camera_level_1 = level_latent_codebook(camera_level[:,0])

    #Auto-encoder starts from here
    encoder_layer_1 = nn.relu(dense_layer(self.embedding_dim)(x))

    if not self.disable_level_latents:
      #Reshape camera level vector
      encoded_camera_level_1 = jnp.expand_dims(encoded_camera_level_1, axis=-2)
      encoded_camera_level_1 = jnp.repeat(encoded_camera_level_1, encoder_layer_1.shape[-2], axis=-2)
      
      #Add positional information to encoder
      encoder_layer_1 = encoder_layer_1 + encoded_camera_level_1

    encoder_layer_2 = dense_layer(self.embedding_dim)(encoder_layer_1) #outputs latent
    #Discretization starts from here
    #Step 1 : Flatten output from encoder_layer_2
    flat_encoded_inputs = jnp.reshape(encoder_layer_2, [-1, self.embedding_dim])

    #Step 2 : Compute distances in the codebook
    ind = jnp.reshape(jnp.array([i for i in range(self.num_codebook_vectors)], dtype=jnp.int32),(self.num_codebook_vectors))
    vector_codebook_jnp = vector_codebook(ind)

    distances = (
        jnp.sum(jnp.square(flat_encoded_inputs), 1, keepdims=True) -
        2 * jnp.matmul(flat_encoded_inputs, jnp.transpose(vector_codebook_jnp)) +
        jnp.transpose(jnp.sum(jnp.square(vector_codebook_jnp), 1, keepdims=True)))
    encoding_indices = jnp.argmax(-distances, 1)
    # encodings = jax.nn.one_hot(encoding_indices,
    #                            self.num_embeddings,
    #                            dtype=jnp.int32)
    
    quantized_embedding = vector_codebook(encoding_indices)
    
    quantized_embedding_reshape = jnp.reshape(quantized_embedding, list(x.shape[0:-1])+[self.embedding_dim])
    # Straight-through estimator.
    quantized_embedding_reshape = encoder_layer_2 + jax.lax.stop_gradient(quantized_embedding_reshape - encoder_layer_2)
    decoder_layer_1 = dense_layer(self.embedding_dim)(quantized_embedding_reshape)
    reconstuced_input = dense_layer(96)(decoder_layer_1)

    e_latent_loss = jnp.mean(
        jnp.square(jax.lax.stop_gradient(quantized_embedding) - flat_encoded_inputs))
    q_latent_loss = jnp.mean(
        jnp.square(quantized_embedding - jax.lax.stop_gradient(flat_encoded_inputs)))
    loss = jnp.mean( (x - reconstuced_input)**2 ) + q_latent_loss + self.commitment_cost * e_latent_loss
    loss = self.lambda_vqvae_loss * loss
    return quantized_embedding_reshape, loss
...
class MLP(nn.Module):
  ...
  latent_layer = 1
  num_latent_layer = 2
  vae_embedding_size:int = 48 #default is 48
  num_camera_levels:int = 1
  fixed_level_latents:bool = True #default
  num_codebook_vectors:int = 36
  disable_level_latents:bool = False
  lambda_vqvae_loss:float = 0.1
  def setup(self):
    ...
    if not self.disable_rgb:
      self.vq_vae = VQ_VAE(num_codebook_vectors=self.num_codebook_vectors, embedding_dim=self.vae_embedding_size, num_camera_levels=self.num_camera_levels, deg_view=self.deg_view, fixed_level_latents=self.fixed_level_latents, disable_level_latents=self.disable_level_latents,
      lambda_vqvae_loss = self.lambda_vqvae_loss)
  ...
  def __call__(self,
    ...
    def predict_density_and_level(means, covs):
      ...
      inputs = x
      num_latent_layer = self.num_latent_layer
     
      if not self.disable_rgb:
        latent_codes, elbo_loss = self.vq_vae(inputs, camera_level)
      else:
        elbo_loss = 0.
      #pdb.set_trace()
      ...
      for i in range(self.net_depth):
        ...
        if not self.disable_rgb and num_latent_layer > 0:
          embeds = dense_layer(self.net_width)(latent_codes)
          x = x + embeds
          num_latent_layer -=1
      ...
      cam_lvl_x = None
      #if not self.disable_rgb:
        #cam_lvl_x = dense_layer(self.num_cam_levels)(x)
        # Note: optax.softmax_cross_entropy already applies softmax
        # cam_lvl_x = nn.softmax(cam_lvl_x)
      ...
      return raw_density, x, cam_lvl_x, elbo_loss

    means, covs = gaussians
    if self.disable_density_normals:
      raw_density, x, cam_lvl_x, raw_elbo_loss = predict_density_and_level(means, covs)
      ...
      predict_density_and_grad_fn = jax.vmap(
          jax.value_and_grad(predict_density_and_level, has_aux=True), in_axes=(0, 0))
      (raw_density_flat, x_flat, cam_lvl_x_flat,_), raw_grad_density_flat = (
          predict_density_and_grad_fn(means_flat, covs_flat))

      # Unflatten the output.

      cam_lvl_x = cam_lvl_x_flat.reshape(means.shape[:-1])
    ...
    if self.disable_rgb:
      ...
      if viewdirs is not None:
        ...
        inputs = x
        num_texture_layer = self.num_latent_layer
        for i in range(self.net_depth_viewdirs):
          x = dense_layer(self.net_width_viewdirs)(x)
          x = self.net_activation(x)
          # if num_texture_layer>0:
          #   texture_embeds = dense_layer(self.net_width_viewdirs)(shape_embedding)
          #   x = x + texture_embeds
          #   num_texture_layer -=1
